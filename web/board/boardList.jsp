...
<%-- <table class="board">
		...
		<c:forEach items="${list }" var="ob">
			<tbody  onclick="location.href='/oatjang/boardView.do?iid=${ob.iid}&pg=${pg }'" class="board_text" id="board_text_id">
				<td class="${ob.iid % 2 ==0 ? 'td_class' : 'td_class2' }" >${ob.iid}</td>  
				<td class="${ob.iid % 2 ==0 ? 'td_class' : 'td_class2' }" id="td_title">${ob.title }</td> 
				<td class="${ob.iid % 2 ==0 ? 'td_class' : 'td_class2' }">${ob.nickname }</td>
				<td class="${ob.iid % 2 ==0 ? 'td_class' : 'td_class2' }">${ob.logtime }</td>
				<td class="${ob.iid % 2 ==0 ? 'td_class' : 'td_class2' }">${ob.hit }</td>
				 <tr height="1" bgcolor="#808080"><td colspan="5"></td></tr>
			</tbody>
		</c:forEach>
		<td colspan="5" align="center"><c:if test="${empty paging}">${paging2.getPagingHTML()}</c:if>
	<c:if test="${!empty paging}">${paging.getPagingHTML()}</c:if></td>
	</table> --%>
